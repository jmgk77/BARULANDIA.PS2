...
int romdisk_find(const char *path, void **ptr, int *size) {
  ...
  // dbglogger_printf("ROMFS: starting at offset 0x%x\n", offset);
  ...
  while (subdir != 0) {
    ...
    // dbglogger_printf("ROMFS: looking for directory '%s' len=%d\n", path,
    // path_len);
    ...
    while (offset != 0) {
      ...
      // dbglogger_printf("ROMFS: comparing against %s\n", fhdr->filename);
    ...
    // dbglogger_printf("ROMFS: directory found at offset 0x%x\n", offset);
  ...
  while (offset != 0) {
    ...
    // dbglogger_printf("ROMFS: looking for file %s at offset 0x%x (%s)\n",
    // path, offset, fhdr->filename);
    if (strcasecmp(fhdr->filename, path) == 0) {
      /* found our file */
      *ptr = (void *)(mounted_img + offset + sizeof(romdisk_file_t) +
                      (strlen(fhdr->filename) & ~0x0f));
      if (size != 0) {
